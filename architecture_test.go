...
const (
	peersModule             = "github.com/sessionbus/grok-peer"
	sdkModule               = "github.com/antst/sessionbus/bus/sdk/go"
	sdkVersion              = "v0.5.7"
	citationCount           = 20
	citationReachabilitySHA = "9acd760a81edb9647e9154f34c3bc236cf4b68b2756d7fbf8885e3feb778b170"
...
func TestRepositoryBoundary(t *testing.T) {
	...
	if got := directoryNames(t, "wrappers"); !equalStrings(got, []string{"grok"}) {
		t.Errorf("wrapper roots = %v, want [grok]", got)
	}
...
func TestSPDXAndLicenseCoverage(t *testing.T) {
	...
	for _, path := range []string{
		"docs/designs/grok-0.5.0/legacy-lane-skill/scripts/lane-preflight",
		"grok/scripts/native-entry",
	} {
...
func TestRepositoryURLsAndRemovedPaths(t *testing.T) {
	for _, root := range []string{"grok", "scripts", "wrappers/README.md"} {
...
func TestRetainedManifestCommandReachability(t *testing.T) {
	...
	skills, err := filepath.Glob("grok/skills/*/SKILL.md")
	if err != nil || len(skills) != 1 || skills[0] != "grok/skills/sessionbus/SKILL.md" {
		t.Fatalf("Grok generic skill inventory = %v (%v)", skills, err)
	}
	var plugin struct{ Name, Version string }
	if err := json.Unmarshal(read(t, "grok/.grok-plugin/plugin.json"), &plugin); err != nil {
		t.Fatal(err)
	}
	if plugin.Name != "sessionbus" || plugin.Version != "0.5.0" {
		t.Errorf("Grok native plugin identity changed: %#v", plugin)
	}
	if !bytes.Contains(read(t, "cmd/grok-peer/main.go"), []byte("grok.PrivateAlias")) {
		t.Error("Grok private alias must dispatch through the same binary")
	}
}

func TestGrokPackageAndInstallerBoundary(t *testing.T) {
	pack := read(t, "scripts/package-product")
	installer := read(t, "scripts/release/install-product")
	for _, exact := range []string{`case "$product" in grok) ;;`, `cp -R "$product/." "$stage/plugin/"`, `ln -s "$product-peer" "$stage/$product-peer-mcp"`, `cp "$product/README.md" "$stage/README.md"`} {
		if !bytes.Contains(pack, []byte(exact)) {
			t.Errorf("Grok archive recipe lacks %q", exact)
		}
	}
	for _, exact := range []string{`case "$product" in grok) ;;`, `ln -sfn grok-peer "$root/grok-peer-mcp"`, `grok plugin uninstall sessionbus --keep-data`, `grok plugin install "$root/plugin" --trust`} {
		if !bytes.Contains(installer, []byte(exact)) {
			t.Errorf("Grok archive installer lacks %q", exact)
		}
	}
	for _, removed := range []string{"internal/", "wrappers/pi", "wrappers/omp", "wrappers/pifamily", "docs/products/", "npm ", "qwen", "kilo", "opencode", "omp-peer", "pi-peer", "package-claude", "package-codex"} {
		if bytes.Contains(pack, []byte(removed)) || bytes.Contains(installer, []byte(removed)) {
			t.Errorf("Grok packaging references removed target %q", removed)
		}
	}
	if !bytes.Contains(read(t, "scripts/install-grok.sh"), []byte("https://github.com/sessionbus/grok-peer/releases/latest")) {
		t.Error("Grok bootstrap does not resolve releases from this repository")
	}
}

func TestReadmeIsTheSourceInstallAuthority(t *testing.T) {
	readme := read(t, "README.md")
	for _, exact := range []string{"scripts/package-product grok ./dist", "grok/README.md", "scripts/install-grok.sh", "`grok-peer-mcp`", "github.com/sessionbus/grok-peer", "docs/migration/FUNCTIONALITY-CHECKLIST.md"} {
